...
for component in "trivy" "cert-oauth2" "suricata" "yara"; do
    ...
    if [ "${!INSTALL_VAR}" = "TRUE" ]; then
        ...
        case "$component" in
            "trivy")
                download_and_verify_file "$WAZUH_TRIVY_REPO_URL/$linux_pattern" "$TMP_FOLDER/install-trivy.sh" "$linux_pattern" "trivy install script" "$WAZUH_TRIVY_REPO_URL/checksums.sha256"
                ...
                ;;
            "cert-oauth2")
                download_and_verify_file "$WAZUH_CERT_OAUTH2_REPO_URL/$linux_pattern" "$TMP_FOLDER/install-cert-oauth2.sh" "$linux_pattern" "cert-oauth2 install script" "$WAZUH_CERT_OAUTH2_REPO_URL/checksums.sha256"
                ...
                ;;
            "suricata")
                download_and_verify_file "$WAZUH_SURICATA_REPO_URL/$linux_pattern" "$TMP_FOLDER/install-suricata.sh" "$linux_pattern" "suricata install script" "$WAZUH_SURICATA_REPO_URL/checksums.sha256"
                ...
                ;;
            "yara")
                download_and_verify_file "$WAZUH_YARA_REPO_URL/$linux_pattern" "$TMP_FOLDER/install-yara-server.sh" "$linux_pattern" "yara install script" "$WAZUH_YARA_REPO_URL/checksums.sha256"
