...
if [ "$UNINSTALL_TRIVY" = "TRUE" ]; then
    ...
    download_and_verify_file "$WAZUH_TRIVY_REPO_URL/$macos_uninstall_pattern" "$TMP_FOLDER/uninstall-trivy.sh" "$macos_uninstall_pattern" "trivy uninstall script" "$WAZUH_TRIVY_REPO_URL/checksums.sha256"
...
if [ "$UNINSTALL_CERT_OAUTH2" = "TRUE" ]; then
    ...
    download_and_verify_file "$WAZUH_CERT_OAUTH2_REPO_URL/$macos_uninstall_pattern" "$TMP_FOLDER/uninstall-cert-oauth2.sh" "$macos_uninstall_pattern" "cert-oauth2 uninstall script" "$WAZUH_CERT_OAUTH2_REPO_URL/checksums.sha256"
...
if [ "$UNINSTALL_SURICATA" = "TRUE" ]; then
    ...
    download_and_verify_file "$WAZUH_SURICATA_REPO_URL/$macos_uninstall_pattern" "$TMP_FOLDER/uninstall-suricata.sh" "$macos_uninstall_pattern" "suricata uninstall script" "$WAZUH_SURICATA_REPO_URL/checksums.sha256"
...
if [ "$UNINSTALL_YARA" = "TRUE" ]; then
    ...
    download_and_verify_file "$WAZUH_YARA_REPO_URL/$macos_uninstall_pattern" "$TMP_FOLDER/uninstall-yara-server.sh" "$macos_uninstall_pattern" "yara uninstall script" "$WAZUH_YARA_REPO_URL/checksums.sha256"
