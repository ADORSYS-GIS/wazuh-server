...
while getopts ":hcsty" opt; do
  case $opt in
    ...
    h) echo "Usage: $0 [-c] [-s] [-y] [-t] [-h]"
       echo ""
       echo "Streamlined Wazuh Agent installation for macOS servers"
       echo ""
       echo "Options:"
...
if [ "$INSTALL_TRIVY" = "TRUE" ]; then
    ...
    download_and_verify_file "$WAZUH_TRIVY_REPO_URL/scripts/macos/install.sh" "$TMP_FOLDER/install-trivy.sh" "scripts/macos/install.sh" "trivy install script" "$WAZUH_TRIVY_REPO_URL/checksums.sha256"
...
if [ "$INSTALL_CERT_OAUTH2" = "TRUE" ]; then
    ...
    download_and_verify_file "$WAZUH_CERT_OAUTH2_REPO_URL/scripts/macos/install.sh" "$TMP_FOLDER/install-cert-oauth2.sh" "scripts/macos/install.sh" "cert-oauth2 install script" "$WAZUH_CERT_OAUTH2_REPO_URL/checksums.sha256"
...
if [ "$INSTALL_SURICATA" = "TRUE" ]; then
    ...
    download_and_verify_file "$WAZUH_SURICATA_REPO_URL/scripts/macos/install.sh" "$TMP_FOLDER/install-suricata.sh" "scripts/macos/install.sh" "suricata install script" "$WAZUH_SURICATA_REPO_URL/checksums.sha256"
...
if [ "$INSTALL_YARA" = "TRUE" ]; then
    ...
    download_and_verify_file "$WAZUH_YARA_REPO_URL/scripts/macos/install.sh" "$TMP_FOLDER/install-yara-server.sh" "scripts/macos/install.sh" "yara install script" "$WAZUH_YARA_REPO_URL/checksums.sha256"
