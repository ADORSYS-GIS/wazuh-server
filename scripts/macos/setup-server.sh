...
if [ "$INSTALL_YARA" = "TRUE" ]; then
    ...
    download_and_verify_file "$WAZUH_YARA_REPO_URL/scripts/macos/install-server.sh" "$TMP_FOLDER/install-yara-server.sh" "scripts/macos/install-server.sh" "yara install script" "$WAZUH_YARA_REPO_URL/checksums.sha256"
