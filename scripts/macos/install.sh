...
start_agent() {
  ...
  info_message "Starting Wazuh agent..."
  if command_exists launchctl; then
    if ! launchctl load -w /Library/LaunchDaemons/com.wazuh.agent.plist; then
        if ! "$WAZUH_CONTROL_PATH" start; then
            error_message "Failed to start Wazuh agent using both launchctl and wazuh-control."
            exit 1
        else
            info_message "Wazuh agent started successfully using wazuh-control."
        fi
    fi
  else
